...
- To send email copies of chat messages and follow notifications, set env vars for your send-push Edge Function:
  - `MAILTRAP_API_TOKEN` — Mailtrap API token
  - `MAILTRAP_FROM_EMAIL` — sender email used in Mailtrap messages
  - `MAILTRAP_FROM_NAME` — optional sender name, defaults to `Flex Pilates Studio`
  - `SENDGRID_API_KEY` — fallback SendGrid API key
  - `SENDGRID_FROM` — fallback verified sender email

For local Supabase auth email testing, the SMTP sample in `supabase/config.toml` uses Mailtrap's sandbox host:
